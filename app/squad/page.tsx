...
import { ClassicFinanceHeader } from '../classic/ClassicFinanceHeader';
...
const BentoCard = ({ title, children, className = '', onExpand }: BentoCardProps) => {
  ...
  return (
    <div
      className={`relative flex flex-col backdrop-blur-md rounded-2xl overflow-hidden group shadow-lg transition-all duration-300 ${
        isHud 
          ? 'ui-bento-surface border border-[var(--ui-panel-border)] shadow-2xl' 
          : 'bg-white border border-gray-200'
      } ${className}`}
    >
      {/* The "Tab" Header - Metallic Gradient Background */}
      <div className={`h-12 border-b flex justify-between items-center px-4 shrink-0 transition-colors ${
        isHud 
          ? 'ui-bento-front border-[var(--ui-divider)]' 
          : 'bg-gray-50 border-gray-100'
      }`}>
        <div className="flex items-center gap-2">
          {/* Decorative 'LED' line */}
          <div className={`h-4 w-1 rounded-full shadow-sm transition-all ${
            isHud ? 'bg-[rgb(var(--ui-accentHot-rgb))] shadow-[0_0_10px_rgba(var(--ui-accentHot-rgb),0.45)]' : 'bg-[#c47c48]'
          }`}></div>
          <span className={`text-[11px] font-bold tracking-widest uppercase transition-colors ${
            isHud ? 'text-gray-200 font-mono' : 'text-gray-900'
          }`}>
            {title}
          </span>
        </div>
        ...
        {typeof onExpand === 'function' ? (
          <button
            ...
            className={`opacity-50 group-hover:opacity-100 transition-all ${
              isHud ? 'hover:text-[#2DD4BF]' : 'hover:text-[#c47c48]'
            }`}
          >
            <Maximize2 size={16} />
          </button>
      ...
      {/* Card Body */}
      <div className="flex-1 p-6 overflow-hidden relative">{children}</div>
    </div>
  );
};
...
const AgentItem = ({
  ...
  return (
    <div
      ...
      className={`group relative p-4 rounded-2xl transition-all duration-300 cursor-pointer border outline-none ${
        isHud
          ? isSelected ? 'bg-[#2A303C]/60 border-[#D97706]/40 shadow-lg' : 'border-transparent hover:bg-[#2A303C]/50 hover:border-[#D97706]/30'
          : isSelected ? 'bg-[#c47c48]/5 border-[#c47c48]/30 shadow-sm' : 'bg-transparent border-transparent hover:bg-gray-50 hover:border-gray-200'
      }`}
    >
      <div className="flex items-center gap-4">
        {/* Avatar (Placeholder for NFT) */}
        <div
          className={`w-11 h-11 rounded-xl ${avatarColor} flex items-center justify-center shadow-md relative overflow-hidden group-hover:scale-105 transition-transform`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-center mb-0.5">
            <h3 className={`text-[14px] font-bold truncate transition-colors ${
              isHud ? 'text-gray-200' : 'text-gray-900'
            }`}>{name}</h3>
            {/* Status Dot */}
            <div
          ...
          <div className="text-[11px] font-bold uppercase tracking-[0.05em] truncate flex items-center gap-2">
            <span className={isHud ? 'text-[#D97706]' : 'text-[#c47c48]'}>{job}</span>
            <span className={isHud ? 'text-gray-600' : 'text-gray-300'}>{'\u2022'}</span>
            <span className={isHud ? 'text-gray-500' : 'text-gray-400'}>{chain}</span>
          </div>
        </div>
      </div>

      {/* "MANAGE" Overlay - Appears on Hover */}
      <div className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 flex items-center justify-center transition-all duration-300 group-hover:backdrop-blur-[1px] ${
        isHud ? 'bg-[#0F1115]/80' : 'bg-white/80'
      }`}>
        <button
          type="button"
          onClick={(e) => {
          ...
          className={`flex items-center gap-2 font-bold text-[11px] tracking-widest px-5 py-2 rounded-full shadow-lg transition-all active:scale-95 border ${
            isHud 
              ? 'text-[#D97706] border-[#D97706] hover:bg-[#D97706]/10' 
              : 'text-[#c47c48] border-[#c47c48] hover:bg-[#c47c48]/10'
          }`}
        >
...
export default function SquadCommandLayout() {
  ...
  return (
    <div className="min-h-screen bg-[var(--ui-bg0)] text-[var(--ui-text)] flex flex-col overflow-hidden transition-colors duration-300">
      {/* ---------------- ZONE 1: TOP NAVIGATION ---------------- */}
      {!isHud ? (
        <ClassicFinanceHeader 
          view="strategies" 
          setView={(v) => v === 'dashboard' ? router.push('/') : null} 
        />
      ) : (
        <header
          className={
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`w-[300px] border-r flex flex-col shrink-0 z-40 transition-colors duration-300 ${isHud ? 'bg-[#13151A] border-[#2A303C]' : 'bg-white border-gray-200'}`}>
          <div className={`p-6 border-b transition-colors ${isHud ? 'border-[#2A303C]/50' : 'border-gray-100'}`}>
            <div className="relative">
              <Search className={`absolute left-3 top-3 transition-colors ${isHud ? 'text-gray-600' : 'text-gray-400'}`} size={16} />
              <input
                type="text"
                placeholder="Search Agents..."
                className={`w-full rounded-xl py-2.5 pl-10 pr-4 text-sm transition-all focus:outline-none ${
                  isHud 
                    ? 'bg-[#0F1115] border border-[#2A303C] text-gray-300 focus:border-[#D97706] placeholder:text-gray-700' 
                    : 'bg-gray-50 border border-gray-200 text-gray-900 focus:border-[#c47c48] placeholder:text-gray-400'
                }`}
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-3 custom-scrollbar">
            <div className={`text-[11px] font-bold uppercase tracking-[0.15em] mb-4 pl-2 ${isHud ? 'text-gray-500' : 'text-gray-400'}`}>
              Active Squad
            </div>
            ...
            <button
              type="button"
              className={`w-full mt-6 border border-dashed rounded-2xl p-6 flex flex-col items-center justify-center transition-all group ${
                isHud 
                  ? 'border-[#2A303C] text-gray-600 hover:text-[#D97706] hover:border-[#D97706]/50 hover:bg-[#D97706]/5' 
                  : 'border-gray-200 text-gray-400 hover:text-[#c47c48] hover:border-[#c47c48]/50 hover:bg-[#c47c48]/5'
              }`}
            >
              <Plus size={28} className="mb-2 group-hover:scale-110 transition-transform" />
              <span className="text-[11px] font-bold uppercase tracking-widest">Deploy New Agent</span>
            </button>
          </div>

          <div className={`p-6 border-t transition-colors ${isHud ? 'border-[#2A303C]' : 'border-gray-100'}`}>
            <div className={`flex justify-between items-center text-[10px] font-bold uppercase tracking-widest ${isHud ? 'text-gray-500' : 'text-gray-400'}`}>
              <span>System Capacity</span>
              <span>3/5 Slots</span>
            </div>
            <div className={`w-full h-1.5 rounded-full mt-3 overflow-hidden ${isHud ? 'bg-[#2A303C]' : 'bg-gray-100'}`}>
              <div className={`h-full transition-all duration-1000 ${isHud ? 'bg-[#D97706]' : 'bg-[#c47c48]'} w-3/5 shadow-sm`}></div>
            </div>
          </div>
        </aside>

        {/* ---------------- ZONE 3: BENTO MAIN STAGE ---------------- */}
        <main className={`flex-1 p-8 overflow-y-auto relative transition-colors duration-300 ${
          isHud 
            ? 'bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-[#1F2937]/20 via-[#0F1115] to-[#0F1115]' 
            : 'bg-[#f0f2f5]'
        }`}>
          {/* Background Grid Pattern (Subtle) */}
          {isHud && <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none"></div>}
